...
const getArea = (widthIn, heightIn) => {
  ...
  const widthFt = Math.floor(widthIn);
  const heightFt = Math.floor(heightIn);
...
function Box({ isPositionOutside, boxProps }) {
  ...
  const [positionOutside, setPositionOutside] = React.useState(
    isPositionOutside
  );
  ...
  const calcRect = (anchorPosition, cursorPosition) => {
    ...
    if (
      ...
      const width =
        ...
        Math.min(anchorPosition.y, cursorPosition.y) +
        1;
      const area = getArea(getInches(width), getInches(height));
      shape = {
        type: "RECTANGLE",
